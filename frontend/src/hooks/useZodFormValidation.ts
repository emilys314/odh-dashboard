...
export function useZodFormValidation<T>(
  ...
  getFieldValidation: (fieldPath?: (string | number)[], ignoreTouchedFields?: boolean) => ZodIssue[];
  ...
  const getFieldValidation = (fieldPath?: (string | number)[], ignoreTouchedFields?: boolean): ZodIssue[] => {
    const key = fieldPath ? fieldPath.join('.') : '/';
